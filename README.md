...
保存配置后可点击 **测试连接**，直接查看供应商、模型和请求耗时；测试失败时会显示
上游 API 返回的具体原因，便于区分密钥、余额、模型名和网络问题。
...
| POST | `/api/auth/password` | 验证当前密码并修改密码 |
| DELETE | `/api/auth/account` | 验证密码并永久删除账号及个人数据 |

除认证接口和 `/health` 外，所有业务 API 都要求有效登录会话。
注册、登录、改密、聊天、文件分析和 API 测试均设置了基础请求频率限制；
会话 Cookie 使用 `HttpOnly`、`SameSite=Lax`，并拒绝跨站修改请求。

### 数据可信度说明

页面顶部会显示数据版本与统一说明。专业目录、学分要求、课程模板和模型生成结果
采用不同的来源等级；课程规划和职业匹配属于规划参考，不代表学校官方结论。
“抢课监控”明确标记为演示模拟数据，“冲突微调”属于决策辅助，不会读取或修改
真实教务系统。答辩演示时应保留这些标识。
